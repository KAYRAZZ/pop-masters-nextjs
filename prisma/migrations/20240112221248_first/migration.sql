...
CREATE TABLE `ParcelTracking` (
    `parcel_tracking_id` INTEGER NOT NULL AUTO_INCREMENT,
    `tracking_number` VARCHAR(191) NOT NULL,
    `figurine_id` VARCHAR(191) NOT NULL,
    `donnees` VARCHAR(191) NULL,
    `date_parcel_added` DATETIME(3) NOT NULL,
    `user_id` INTEGER NOT NULL,
    `collectionCollection_id` INTEGER NULL,

    PRIMARY KEY (`parcel_tracking_id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `ParcelTracking` ADD CONSTRAINT `ParcelTracking_collectionCollection_id_fkey` FOREIGN KEY (`collectionCollection_id`) REFERENCES `Collection`(`collection_id`) ON DELETE SET NULL ON UPDATE CASCADE;
